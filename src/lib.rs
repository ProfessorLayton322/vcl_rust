...
fn compare_vec4f(vec: &Vec4f, expected : [f32; 4]) {
...
fn test_basic() {
    ...
    let c = a + b;
    assert!(c, [33.0, 34.0, 35.0, 36.0]);

    let arr : [f32; 4] = [-2.0, 1.0, 3.0, -4.0];
    let mut d = Vec4f::default();
    d.load(arr.as_ptr());
    compare_vec4f(&d, [-2.0, 1.0, 3.0, -4.0]);
}
...
fn test_arithmetic() {
    ...
    compare_vec4f(&a, [1.0, -1.0, 2.0, 3.0]);
    a *= Vec4f::new(3.0, -2.0, 5.0, -1.0);
    compare_vec4f(&a, [3.0, 2.0, 10.0, -3.0]);
    a /= Vec4f::new(-1.0, 2.0, -2.0, -3.0);
    compare_vec4f(&a, [-3.0, 1.0, -5.0, 1.0]);

    let b = a + Vec4f::new(1.0, 2.0, 1.0, 2.0);
    compare_vec4f(&b, [-2.0, 3.0, -4.0, 3.0]);
    let c = Vec4f::new(1.0, -2.0, 7.0, -3.0) - b;
    compare_vec4f(&c, [3.0, -5.0, 11.0, -6.0]);
    let d = Vec4f::new(2.0, -1.0, -2.0, 4.0) * c;
    compare_vec4f(&d, [6.0, 5.0, -22.0, -24.0]);
    let e = d / Vec4f::new(3.0, -5.0, -11.0, -3.0);
    compare_vec4f(&e, [2.0, -1.0, 2.0, 8.0]);

    assert_eq!(horizontal_add(e), 11.0);

    let first = Vec4f::new(5.0, -2.0, 3.0, 1.0);
    let second = Vec4f::new(1.0, 2.0, 2.0, 11.0);
    compare_vec4f(&max(first, second), [5.0, 2.0, 3.0, 11.0]);
    compare_vec4f(&min(first, second), [1.0, -2.0, 2.0, 1.0]);

    let third = Vec4f::new(-2.0, 3.0, 2.0, -1.0);
    compare_vec4f(&abs(third), [2.0, 3.0, 2.0, 1.0]);
    compare_vec4f(&(-third), [2.0, -3.0, -2.0, 1.0]);

    compare_vec4f(&change_sign::<true, false, true, false>(third), 
        2.0, 3.0, -2.0, -1.0);

    compare_vec4f(&sign_combine(
        Vec4f::new(-2.0, -1.0, 0.0, 1.0),
        Vec4f::new(-10.0, 0.0, -20.0, 30.0)),
        [2.0, -1.0, -0.0, 1.0]);

    compare_vec4f(&sqrt(Vec4f::new(0.0, 1.0, 2.0, 3.0)),
        [0.0, 1.0, f32::sqrt(2.0), f32::sqrt(3.0)]);

    compare_vec4f(&square(Vec4f::new(-1.0, 1.0, 2.0, 3.0)),
        [1.0, 1.0, 4.0, 9.0]);

    compare_vec4f(&pow(Vec4f::new(-1.0, 2.0, 3.0, 1.5), 2),
        [1.0, 4.0, 9.0, 1.5 * 1.5]);
    compare_vec4f(&pow(Vec4f::new(-1.0, 2.0, 3.0, 1.5), -1),
        [-1.0, 0.5, 1.0 / 3.0, 1.0 / 1.5]);

    compare_vec4f(&round(Vec4f::new(1.0, 1.4, 1.5, 1.6)),
        [1.0, 1.0, 2.0, 2.0]);
}
...
fn test_bitwise() {
    ...
    compare_vec4f(&a, [2.0, 4.0, 8.0, 0.0]);
    ...
    compare_vec4f(&a, [3.0, 4.0, 16.0, 3.0]);
    ...
    a ^= Vec4f::new(2.0, 5.0, 7.0, -2.0);
    compare_vec4f(&a, [0.0, 0.0, 0.0, 0.0]);
...
fn test_insert_extract() {
    ...
    a.insert(2, -2.0);
    a.insert(3, 23.0);
    compare_vec4f(&a, [6.0, 7.0, -2.0, 23.0]);
    a.insert(2, 5.0);
    compare_vec4f(&a, [6.0, 7.0, 5.0, 23.0]);
...
fn test_load_store() {
    ...
    let mut a = Vec4f::default();
    a.load(arr.as_ptr());
    compare_vec4f(&a, [-2.0, 1.0, 4.0, 5.0]);
    ...
    compare_vec4f(&b, [-1.0, 2.0, 3.0, -2.0]);
    ...
    compare_vec4f(&c, [0.0, 0.0, 0.0, 0.0]);
    c.load_partial(1, another_arr.as_ptr());
    compare_vec4f(&c, [-10.0, 0.0, 0.0, 0.0]);
    c.load_partial(2, another_arr.as_ptr());
    compare_vec4f(&c, [-10.0, 3.0, 0.0, 0.0]);
    c.load_partial(3, another_arr.as_ptr());
    compare_vec4f(&c, [-10.0, 3.0, -2.0, 0.0]);
    c.load_partial(4, another_arr.as_ptr());
    compare_vec4f(&c, [-10.0, 3.0, -2.0, 7.0]);

    let mut d = Vec4f::new(-3.0, 2.0, 1.0, 11.0);
    d.load_partial(5, another_arr.as_ptr());
    compare_vec4f(&d, [-3.0, 2.0, 1.0, 11.0]);
    d.cutoff(3);
    compare_vec4f(&d, [-3.0, 2.0, 1.0, 0.0]);
    d.cutoff(2);
    compare_vec4f(&d, [-3.0, 2.0, 0.0, 0.0]);
    d.cutoff(1);
    compare_vec4f(&d, [-3.0, 0.0, 0.0, 0.0]);
    d.cutoff(0);
    compare_vec4f(&d, [0.0, 0.0, 0.0, 0.0]);
}
